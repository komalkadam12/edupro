...
<input type="file" name="file_name" class="form-control file2 inline btn btn-primary btn-sm" data-label="<i class='glyphicon glyphicon-file'></i> Browse" / required>
                    ...
                    <div class="form-group">
						<div class="col-sm-offset-3 col-sm-5">
							<button type="submit" class="btn btn-info btn-sm btn-rounded"> <i class="fa fa-plus"></i>&nbsp;<?php echo get_phrase('add_study_material');?></button>
						</div>
					</div>
					<br>
                <?php echo form_close();?>	
...
<table id="example23" class="display nowrap" cellspacing="0" width="100%">
    ...
    <tbody>
        <?php 
                                $study_material	=	$this->db->get_where('document', array('teacher_id' => $this->session->userdata('login_user_id')))->result_array();
            ...
            <tr>
                ...
                <td>
				 ...
				 <a href="<?php echo base_url().'uploads/document/'.$row['file_name']; ?>"><button type="button" class="btn btn-info btn-circle btn-xs"><i class="fa fa-download"></i></button></a>

                </td>
                <td>
				
				                       <a  onclick="showAjaxModal('<?php echo base_url();?>modal/popup/modal_study_material_edit/<?php echo $row['document_id']?>');" ><button type="button" class="btn btn-success btn-circle btn-xs"><i class="fa fa-pencil"></i></button></a>
				
                   
					 <a href="<?php echo base_url();?>teacher/study_material/delete/<?php echo $row['document_id']?>" ><button type="button" class="btn btn-danger btn-circle btn-xs" onclick="return confirm('Are you sure to delete?');"><i class="fa fa-times"></i></button></a>
